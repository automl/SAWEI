...
class DACBOEnv(AbstractEnv):
    ...
    def build_solver(self):
        ...
        # Build intensifier
        intensifier_kwargs = smac_kwargs.get("intensifier", None)
        if intensifier_kwargs:
            intensifier = self.smac_class.get_intensifier(scenario=scenario, **intensifier_kwargs)
            smac_kwargs["intensifier"] = intensifier
        ...
        if "logging_level" in smac_kwargs:
            p = smac_kwargs["logging_level"]
            cwd = os.getcwd()
            p_ = Path(cwd.split("AWEI/awei_runs")[0]) / p  # TODO use regex for this
            smac_kwargs["logging_level"] = p_
